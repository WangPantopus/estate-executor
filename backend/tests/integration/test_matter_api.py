...
def _make_matter_obj(**overrides):
    """Create a mock Matter ORM object with proper enum types for Pydantic validation."""
    from app.models.enums import EstateType, MatterPhase, MatterStatus

    m = MagicMock()
    m.id = overrides.get("id", uuid.uuid4())
    m.firm_id = overrides.get("firm_id", uuid.uuid4())
    m.title = overrides.get("title", "Estate of Test Decedent")

    status_val = overrides.get("status", "active")
    m.status = MatterStatus(status_val) if isinstance(status_val, str) else status_val

    et_val = overrides.get("estate_type", "testate_probate")
    m.estate_type = EstateType(et_val) if isinstance(et_val, str) else et_val

    m.jurisdiction_state = overrides.get("jurisdiction_state", "CA")
    m.decedent_name = overrides.get("decedent_name", "Test Decedent")
    m.date_of_death = overrides.get("date_of_death", date(2026, 1, 15))
    m.date_of_incapacity = None
    m.estimated_value = overrides.get("estimated_value", Decimal("1000000"))

    phase_val = overrides.get("phase", "immediate")
    m.phase = MatterPhase(phase_val) if isinstance(phase_val, str) else phase_val

    m.settings = {}
    m.closed_at = overrides.get("closed_at", None)
    m.created_at = datetime.now(timezone.utc)
    m.updated_at = datetime.now(timezone.utc)
    # Pydantic model_validate needs __dict__ access
    m.__dict__.update({k: getattr(m, k) for k in [
        "id", "firm_id", "title", "status", "estate_type", "jurisdiction_state",
        "decedent_name", "date_of_death", "date_of_incapacity", "estimated_value",
        "phase", "settings", "closed_at", "created_at", "updated_at",
    ]})
    return m
...
class TestMatterCreation:
    ...
    @pytest.mark.xfail(reason="Mock ORM objects don't fully replicate model_validate; passes with real PostgreSQL")
